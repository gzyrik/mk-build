...
public:
    ...
    uint32_t getVideoChunkToWriter();
protected:
    virtual ~MPEG4Writer();
...
#if 1
    size_t  mFileCacheSize;                     // the size of cache.
    uint8_t *mFileCacheBuffer;              // the start addr of cache.
    size_t  mFileCacheDataLength;        // the length of data in cache.
#endif
...
#if 1
    size_t createFileCache();
    size_t destroyFileCache();
    size_t write2FileCache( uint8_t *src_data, size_t src_size);
    size_t flushFileCache();
#endif
