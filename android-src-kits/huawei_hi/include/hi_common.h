...
typedef struct hiSYS_CHIP_ATTR_S
{
    ...
    HI_U64  u64ChipID;                     /**<the unique chipid*//**<CNcomment:芯片唯一ID */
}HI_SYS_CHIP_ATTR_S;
...
/**
@brief search MMZ. CNcomment: 通过mmz块的中间的某个虚拟地址获取mmz块的物理首地址、虚拟首地址、mmz的尺寸 CNend
@attention \n
@param[in] pVir    Virtual address of a buffer CNcomment: buffer的某个中间虚拟地址 CNend
@param[out] pu32Viraddr_S   Virtual address of a buffer  CNcomment: buffer的虚拟首地址 CNend
@param[out] pu32Phyaddr_S   Physical address of a buffer  CNcomment: buffer的物理首地址 CNend
@param[out] pu32Size        Size of a buffer CNcomment: buffer空间大小 CNend
@retval ::HI_SUCCESS Success CNcomment: 成功 CNend
@retval ::HI_FAILURE Fail. CNcomment: 失败 CNend
@see \n
N/A CNcomment: 无 CNend
